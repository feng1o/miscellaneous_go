...
func main() {
	...
	*/
	...
	db, err := sql.Open("mysql", "root:feng123@tcp(127.0.0.1:3306)/test?charset=utf8") //第一个参数为驱动名
	checkErr(err)
	...
	var name1 string = "'joxtest'"
	rows, err := db.Query("insert into tmpdb.tmptab values(101, '姓名1', 'address1'), (102, " + name1 + ", 'address2'), (103, '姓名3', 'address3'), (104, '姓名4', 'address4')")
	checkErr(err)
	fmt.Println(rows)
